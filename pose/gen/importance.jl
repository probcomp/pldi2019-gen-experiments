...
custom_proposal_latent_images = [custom_proposal_importance_resampling() for _=1:4]

# run sampling importance resampling with the prior proposal
function generic_proposal_importance_resampling()
    constraints = choicemap((:image, image))
    (inferred_trace, _) = importance_resampling(
        generative_model, (), constraints, 1000)
    (latent_image, _, _) = get_retval(inferred_trace)
    latent_image
end

generic_proposal_latent_images = [generic_proposal_importance_resampling() for _=1:4]

# show the observed image and the reconstruction
top_row = hcat(ground_truth_latent_image, image, zero(image), zero(image))
